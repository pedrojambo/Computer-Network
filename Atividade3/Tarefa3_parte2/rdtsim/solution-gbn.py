##
## SAMPLE SOLUTION --- GO-BACK-N PROTOCOL --- APRIL 10
##
## Python version of:
##
## ****************************************************************************
...
class EntityA:
    ...
    def envia_msgs(self):
        while (self.layer5_msgs
               and len(self.pacotes_enviados) < self.janela):
            mensagem = self.layer5_msgs.pop(0)
            seqnum = self.muda_seqnum()
            novo_pacote = Pkt(seqnum, 0, 0, mensagem.data)
    ...
    def timer_interrupt(self):
        if not self.prosseguir:
            self.tentativas += 1
            if TRACE>0:
                print(f'[A:base {self.base}] Rats!  Made no progress for {self.tentativas} timeouts.')
        self.prosseguir = False
        # print(f'[A:base {self.base}] Resending {len(self.pacotes_enviados)} packets.')
