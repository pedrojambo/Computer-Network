...
class EntityA:
    ...
    def __init__(self, seqnum_limit):
        ...
        self.WAIT_TIME = 10.0 * (seqnum_limit//2)
        self.janela = seqnum_limit//2
        self.base = 0
        self.layer5_msgs = []
    ...
    def input(self, packet):
        if TRACE>0:
                print('A: ACK '+ str(packet.acknum) +' recebido')

        if pkt_is_corrupt(packet):
            if TRACE>0:
                print('A: ACK '+ str(packet.acknum) +' fora de ordem/corrompido')
            return

        i = 0
        while i < len(self.pacotes_enviados): 
            if self.pacotes_enviados[i].seqnum != packet.acknum:
                i += 1
                continue

            self.base += i+1
            self.pacotes_enviados = self.pacotes_enviados[i+1:]
            self.prosseguir = True
            self.tentativas = 0
            stop_timer(self)

            if self.pacotes_enviados:
    ...
    def timer_interrupt(self):
        if TRACE>0:
            print('A: timer disparou')
        if not self.prosseguir:
            self.tentativas += 1
        self.prosseguir = False

        for p in self.pacotes_enviados:
            if TRACE>0:
                print ('A: pacote '+ str(p.seqnum) +' enviado novamente')
            to_layer3(self, p)
        start_timer(self, self.WAIT_TIME * (self.tentativas+1))

    def muda_seqnum(self):
        return (self.base + len(self.pacotes_enviados)) % self.seqnum_limit
    
    def envia_msgs(self):
        while self.layer5_msgs and len(self.pacotes_enviados) < self.janela:
            m = self.layer5_msgs.pop(0)
            seqnum = self.muda_seqnum()
            novo_pacote = Pkt(seqnum, 0, 0, m.data)
            pkt_insert_checksum(novo_pacote)
            self.pacotes_enviados.append(novo_pacote)
            if TRACE>0:
                print ('A: pacote '+ str(novo_pacote.seqnum) +' enviado')
            to_layer3(self, novo_pacote)

            if len(self.pacotes_enviados) == 1:
                start_timer(self, self.WAIT_TIME)
...
class EntityB:
    ...
    def __init__(self, seqnum_limit):
        ...
        self.ultimo_ack = seqnum_limit-1

        self.seqnum_esperado = 0

    # Called from layer 3, when a packet arrives for layer 4 at EntityB.
    # The argument `packet` is a Pkt containing the newly arrived packet.
    def input(self, packet):
        if (pkt_is_corrupt(packet)
            or packet.seqnum != self.seqnum_esperado):
            if TRACE>0:
                print('B: pacote '+ str(packet.seqnum) +' fora de ordem/corrompido')
            p = Pkt(0, self.ultimo_ack, 0, packet.payload)
            pkt_insert_checksum(p)
            if TRACE>0:
                print ('B: ACK '+ str(p.acknum) +' enviado novamente')
            to_layer3(self, p)
        else:
            if TRACE>0:
                print ('B: pacote '+ str(packet.seqnum) +' recebido')
            to_layer5(self, Msg(packet.payload))
            p = Pkt(0, self.seqnum_esperado, 0, packet.payload)
            pkt_insert_checksum(p)
            if TRACE>0:
                print ('B: ACK '+ str(p.acknum) +' enviado')
            to_layer3(self, p)
            self.ultimo_ack = self.seqnum_esperado
            self.seqnum_esperado = self.next_seqnum_esperado()

    def next_seqnum_esperado(self):
        return (self.seqnum_esperado + 1) % self.seqnum_limit
