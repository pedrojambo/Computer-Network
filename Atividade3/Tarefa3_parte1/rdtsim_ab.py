...
class EntityA:
    ...
    def input(self, packet):
        if TRACE>0:
                print('\n A: ACK '+ str(packet.acknum) +' recebido')
    ...
    def handle_event_wait_for_ack(self, e, arg=None):
        if e==self.OUTPUT:
            ...
            if (pkt_is_corrupt(p)
                ...
                if TRACE>0:
                    print('\n A: ACK '+ str(p.acknum) +' fora de ordem/corrompido')
                return
            stop_timer(self)
            #
            self.bit = 1 - self.bit
            ...
            if TRACE>0:
                print('\n A: pacote '+ str(self.sent_pkt.seqnum) +' reenviado')
...
class EntityB:
    ...
    def input(self, packet):
        ...
        if (packet.seqnum != self.expecting_bit
            ...
            if TRACE>0:
                print('\n B: pacote '+ str(packet.seqnum) +' fora de ordem/corrompido')
            p = Pkt(0, 1-self.expecting_bit, 0, packet.payload)
            pkt_insert_checksum(p)
            if TRACE>0:
                print('\n B: ACK '+ str(p.acknum) +' reenviado')
            to_layer3(self, p)
        else:
            to_layer5(self, Msg(packet.payload))
            if TRACE>0:
                print('\n B: pacote '+ str(packet.seqnum) +' recebido')
            # Ack.
            p = Pkt(0, self.expecting_bit, 0, packet.payload)
            pkt_insert_checksum(p)
            if TRACE>0:
                print('\n B: ACK '+ str(p.acknum) +' enviado')
            to_layer3(self, p)
            #
